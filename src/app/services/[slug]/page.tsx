...
// Allow dynamic slug params beyond what generateStaticParams provides
export const dynamicParams = true;

export async function generateStaticParams() {
    // Use a direct client for build-time (no cookies available)
...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <>
            {/* 1. Service Hero */}
            <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 bg-[var(--theme-primary,#1E3A5F)]">
                <div className="absolute inset-0 z-0 opacity-20">
                    <Image
                        ...
                        priority
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[var(--theme-primary,#1E3A5F)] to-transparent" />
                </div>
                    ...
                    <div className="grid lg:grid-cols-3 gap-12 lg:gap-20">
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            <div className="prose prose-lg prose-slate max-w-none">
                                <h2 className="text-3xl font-bold text-[var(--theme-primary,#1E3A5F)] mb-6">Service Overview</h2>
                            ...
                            {featuresList.length > 0 && (
                                <div>
                                    <h3 className="text-2xl font-bold text-[var(--theme-primary,#1E3A5F)] mb-6">Key Capabilities</h3>
                        ...
                        <div className="space-y-8">
                            <div className="bg-slate-50 p-8 rounded-2xl border border-slate-100 shadow-sm sticky top-28">
                                <h3 className="text-xl font-bold text-[var(--theme-primary,#1E3A5F)] mb-6">Need this service?</h3>
                                <p className="text-slate-600 mb-6 text-sm leading-relaxed">
                                    Contact our experts today for a comprehensive consultation and project estimate tailored to your requirements.
                                </p>

                                <Button asChild className="w-full bg-[var(--theme-accent,#2563eb)] hover:opacity-90 mb-6 h-12 text-base text-white">
                                ...
                                <div className="space-y-4 pt-6 border-t border-slate-200">
                                    {company?.phone_primary && (
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 bg-white rounded-full flex items-center justify-center text-[var(--theme-accent,#2563eb)] shadow-sm">
                                                <Phone className="h-5 w-5" />
                                            </div>
                                            <div>
                                                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Call Us Now</div>
                                                <a href={`tel:${company.phone_primary}`} className="font-bold text-[var(--theme-primary,#1E3A5F)] hover:text-[var(--theme-accent,#2563eb)]">
                                                    {company.phone_primary}
                                                </a>
                                            </div>
                                        </div>
                                    ...
                                    {company?.email_primary && (
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <a href={`mailto:${company.email_primary}`} className="font-bold text-[var(--theme-primary,#1E3A5F)] hover:text-[var(--theme-accent,#2563eb)] text-sm">
                                                    {company.email_primary}
                                                </a>
                                            </div>
                                        </div>
