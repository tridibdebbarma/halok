...
export default async function Home() {
  ...
  // About us highlights from DB or fallback
  const aboutHighlights = (company?.about_highlights && company.about_highlights.length > 0)
    ? company.about_highlights
    : [
  ...
  return (
    <>
      ...
      <section className="relative h-[85vh] min-h-[600px] flex items-center pt-16 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-white">
          <div className="max-w-3xl space-y-6 animate-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <Button asChild size="lg" className="bg-[var(--theme-accent,#2563eb)] hover:opacity-90 text-base h-12 px-8 text-white">
        ...
        {/* Info Banner floating at bottom of hero — increased padding and spacing */}
        <div className="absolute bottom-0 left-0 right-0 z-20 hidden md:block translate-y-1/2">
          <div className="container mx-auto px-4">
            <div className="bg-white rounded-xl shadow-xl flex items-stretch divide-x divide-slate-100 p-3 border border-slate-100">
              <div className="flex-1 flex items-center gap-5 px-8 py-7 hover:bg-slate-50 transition-colors rounded-l-lg">
                <div className="h-14 w-14 rounded-full bg-blue-50 text-[var(--theme-accent,#2563eb)] flex items-center justify-center shrink-0">
                  <ShieldCheck className="h-7 w-7" />
                </div>
                <div>
                  <h3 className="font-bold text-[var(--theme-primary,#1E3A5F)] text-lg">Certified Quality</h3>
                  <p className="text-sm text-slate-500 mt-1">ISO 9001:2015 Compliant</p>
                </div>
              </div>
              <div className="flex-1 flex items-center gap-5 px-8 py-7 hover:bg-slate-50 transition-colors">
                <div className="h-14 w-14 rounded-full bg-blue-50 text-[var(--theme-accent,#2563eb)] flex items-center justify-center shrink-0">
                  <HardHat className="h-7 w-7" />
                </div>
                <div>
                  <h3 className="font-bold text-[var(--theme-primary,#1E3A5F)] text-lg">{company?.years_of_experience || 15}+ Years</h3>
                  <p className="text-sm text-slate-500 mt-1">Industry Experience</p>
                </div>
              </div>
              <div className="flex-1 flex items-center gap-5 px-8 py-7 hover:bg-slate-50 transition-colors rounded-r-lg">
                <div className="h-14 w-14 rounded-full bg-blue-50 text-[var(--theme-accent,#2563eb)] flex items-center justify-center shrink-0">
                  <Clock className="h-7 w-7" />
                </div>
                <div>
                  <h3 className="font-bold text-[var(--theme-primary,#1E3A5F)] text-lg">On Time Delivery</h3>
      ...
      {/* 2. About Us Sneak Peek — increased top padding for info banner clearance */}
      <section className="py-24 md:pt-44 md:pb-24 bg-slate-50">
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div className="relative aspect-square md:aspect-auto md:h-[600px] rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="absolute bottom-6 left-6 bg-white p-6 rounded-xl shadow-lg flex items-center gap-4 max-w-[280px]">
                <div className="text-4xl font-extrabold text-[var(--theme-accent,#2563eb)] leading-none">
                  {company?.years_of_experience || 15}
                </div>
                <div className="text-sm font-semibold text-[var(--theme-primary,#1E3A5F)] leading-tight">
            ...
            <div className="space-y-6">
              <div className="inline-block px-4 py-1.5 bg-blue-50 text-[var(--theme-accent,#2563eb)] font-semibold text-sm rounded-full">
                About Our Company
              </div>
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--theme-primary,#1E3A5F)] leading-tight">
              ...
              <ul className="space-y-4 pt-4">
                {aboutHighlights.map((item: string, i: number) => (
              ...
              <div className="pt-6">
                <Button asChild size="lg" className="bg-[var(--theme-primary,#1E3A5F)] hover:opacity-90 text-white">
      ...
      <section className="py-24 bg-white relative">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <h2 className="text-3xl md:text-5xl font-bold text-[var(--theme-primary,#1E3A5F)]">Our Core Capabilities</h2>
              ...
              services.map((service: any) => (
                <Card key={service.id} className="group border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full bg-slate-50/50 hover:bg-white">
                  <div className="relative h-64 overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1 text-xs font-bold text-[var(--theme-accent,#2563eb)] rounded-full shadow-sm">
                      {service.category}
                    </div>
                  </div>
          ...
          <div className="text-center mt-12">
            <Button asChild variant="outline" size="lg" className="border-blue-200 text-[var(--theme-accent,#2563eb)] hover:bg-blue-50">
      ...
      {/* 4. CTA Section */}
      <section className="py-24 bg-[var(--theme-primary,#1E3A5F)] relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
              <Button asChild size="lg" className="bg-white text-[var(--theme-primary,#1E3A5F)] hover:bg-slate-100 h-14 px-8 text-lg w-full sm:w-auto">
